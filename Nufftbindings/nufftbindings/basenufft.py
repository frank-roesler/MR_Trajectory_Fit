...
class baseNUFFT:
    def set_dims(self, K, dims, device, Nb=1, doublePrecision=False):
        ...
        self.nx = self.dims[0]
        self.xx = torch.arange(self.nx, device=self.device, dtype=self.torch_dtype) - self.nx / 2.0
        if self.ndim >= 2:
            self.ny = self.dims[1]
            self.xy = torch.arange(self.ny, device=self.device, dtype=self.torch_dtype) - self.ny / 2.0
        if self.ndim == 3:
            self.nz = self.dims[2]
            self.xz = torch.arange(self.nz, device=self.device, dtype=self.torch_dtype) - self.nz / 2.0
    ...
    def test_f(self, f):
        if f.shape[0] != self.Nbatch and f.shape[0] != 1:
            raise Exception(
                "The batch size does not correspond to the one indicated in set_dims. Expected the first dimension to be of size " + str(self.Nbatch) + " but got shape[0]=" + str(f.shape[0])
            )
