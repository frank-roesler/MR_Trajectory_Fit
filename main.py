...
model = FourierCurve(tmin=0, tmax=params["duration"], initial_max=kmax_traj, n_coeffs=params["model_size"], coeff_lvl=1e-2)
...
for step in range(params["train_steps"]):
    ...
    plotter.print_info(step, image_loss, grad_loss, slew_loss, *derivatives)
