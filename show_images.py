...
checkpointer.params["timesteps"] = params["timesteps"] // 2 + 1
checkpointer.dt = checkpointer.params["duration"] / (checkpointer.params["timesteps"] - 1)
...
with torch.no_grad():
    t_json = torch.linspace(0, checkpointer.params["duration"], steps=checkpointer.params["timesteps"]).unsqueeze(1)
    traj_json = model(t_json)  # (timesteps, 2)
    rosette_json, _, _ = make_rosette(traj_json, rotation_matrix, checkpointer.params["n_petals"], kmax_img, checkpointer.dt, zero_filling=checkpointer.params["zero_filling"])

export_k_as_csv(traj, path)
checkpointer.export_json(rosette_json)
