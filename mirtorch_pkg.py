...
from mirtorch.linear import LinearMap, FFTCn, NuSense, Sense, FFTCn, Identity, Diff2dgram, Gmri, Wavelet2D, NuSenseGram, Diff3dgram


class NuSense_om(LinearMap):
    """
    The gram operator $A'A$
    Attributes:
    ...
    def __init__(self, smaps: Tensor, traj: Tensor, norm="ortho", numpoints: Union[int, Sequence[int]] = 6, grid_size: float = 2):
        self.traj = traj
        self.A = NuSense(smaps, traj, norm=norm, batchmode=True, numpoints=numpoints, grid_size=grid_size, sequential=False)
        ...
        class Aadjoint(torch.autograd.Function):
            ...
            def backward(ctx, x):
                ...
                dldom = torch.zeros_like(ktraj)
                #                 print(x)
                #                 print('x', torch.sum(torch.abs(x)))
                #                 plt.figure()
                #                 plt.imshow(torch.abs(x)[0,0,:,:,].cpu().numpy())
                #                 plt.colorbar()
                for i in range(len(im_size)):
                    if len(im_size) == 1:
        ...
        super(NuSense_om, self).__init__(tuple(self.A.size_in), tuple(self.A.size_out))
    ...
    def __init__(self, smaps: Tensor, traj: Tensor, norm="ortho", numpoints: Union[int, Sequence[int]] = 6, grid_size: float = 2):
        ...
        class Aforward(torch.autograd.Function):
            ...
            def forward(ctx, x, ktraj, AHA, A):
                ...
                ctx.AHA = AHA
                ctx.A = A
                # print('allocate forward gram', torch.cuda.max_memory_allocated())
                return AHA * x
            ...
            def backward(ctx, dx):
                ...
                for i in range(len(im_size)):
                    ...
                    if len(im_size) == 2:
                        r = torch.arange(im_size[i]).expand(1, 1, 1, -1).permute(0, 1, 3 - i, 2 + i).to(device=dx.device)
                    if len(im_size) == 3:
        ...
        super(NuSenseGram_om, self).__init__(tuple(self.A.size_in), tuple(self.A.size_in))
...
class Gram_inv(LinearMap):
    def __init__(self, smaps: Tensor, traj: Tensor, lambd: float, tol: float, max_iter=100, norm="ortho", numpoints: Union[int, Sequence[int]] = 6, grid_size: float = 2, alert=False, x0=None):
        self.traj = traj
        batchmode = True
        ...
        class Aforward(torch.autograd.Function):
            ...
            def backward(ctx, dx):
                ...
                for i in range(len(im_size)):
                    ...
                    Pbr = Pb * r
                    APbr = ctx.A * Pbr
                    dldwd = (torch.conj(APbr).mul_((0 - 1j)).mul_(APdx).add_(APdxr.mul_(torch.conj(APb)).mul_(0 + 1j))).real
                    dldom[:, i, :] = torch.sum(torch.sum(dldwd, dim=1), dim=0)
        ...
        super(Gram_inv, self).__init__(tuple(self.A.size_in), tuple(self.A.size_in))
...
class Gram_inv_diff(LinearMap):
    def __init__(self, smaps: Tensor, traj: Tensor, lambd: float, tol: float, max_iter=100, norm="ortho", numpoints: Union[int, Sequence[int]] = 6, grid_size: float = 2, alert=False, x0=None):
        self.traj = traj
        batchmode = True
        self.A = NuSense(smaps, traj, norm=norm, batchmode=batchmode, numpoints=numpoints, grid_size=grid_size)
        im_size = smaps.shape[2:]
        T = Diff3dgram(self.A.size_in)
        self.AHA = NuSenseGram(smaps, traj, norm=norm, batchmode=batchmode, numpoints=numpoints, grid_size=grid_size) + lambd * T
        # self.AHA = self.A.H*self.A + lambd * T
        ...
        class Aforward(torch.autograd.Function):
            ...
            def backward(ctx, dx):
                ...
                for i in range(len(im_size)):
                    ...
                    del Pdxr
                    del Pbr
                    # dldwd = ((0 - 1j) * torch.conj(APbr) * APdx + (0 + 1j) * torch.conj(APb) * APdxr).real
                    dldwd = (torch.conj(APbr).mul_((0 - 1j)).mul_(APdx).add_(APdxr.mul_(torch.conj(APb)).mul_(0 + 1j))).real
                    dldom[:, i, :] = torch.sum(torch.sum(dldwd, dim=1), dim=0)
                return Pdx, dldom, None, None, None
        ...
        super(Gram_inv_diff, self).__init__(tuple(self.A.size_in), tuple(self.A.size_in))
