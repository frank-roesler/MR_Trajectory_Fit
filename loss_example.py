...
def logb_star_loss(pns, params, delta = 0.01):
    x = pns
...
def pns_loss(pns, params, mode="exp", delta=0.01):
    if mode == "exp":
        ...
        pns_loss = effective_potential_loss(pns, params["pns_threshold"], epsilon=0.5)
    return pns_loss

pns_example = np.linspace(80, 105, 100)

pns_loss_exp = pns_loss(pns_example, params, mode="exp")
pns_loss_threshold = pns_loss(pns_example, params, mode="threshold")
pns_loss_logb_star_001 = pns_loss(pns_example, params, mode="logb_star", delta=0.01)
pns_loss_logb_star_005 = pns_loss(pns_example, params, mode="logb_star", delta=0.05)
...
plt.plot(pns_example, pns_loss_logb_star_001, label='LogB* Loss (Delta=0.01)')
plt.plot(pns_example, pns_loss_logb_star_005, label='LogB* Loss (Delta=0.05)')
...
plt.ylabel('Loss')
plt.ylim(min(pns_loss_effective_potential) * 1.5, max(pns_loss_threshold) * 1.5) 
plt.legend()
plt.grid()
plt.show()
